...
const LOGIN_DRAFT_KEY = 'ttc_polotno_login_draft_v1';
...
export async function getAccountStatus() {
  return normalizeResponse(await ajax('ttc_polotno_account_nonce', {}));
}
...
export function saveLoginDraft({ designJson, title = 'Untitled Design' }) {
  try {
    ...
    console.warn('Could not preserve unsaved design before login:', err);
  }
}

export function consumeLoginDraft({ maxAgeMs = 24 * 60 * 60 * 1000 } = {}) {
  try {
    const raw = window.localStorage.getItem(LOGIN_DRAFT_KEY);
    if (!raw) return null;

    const draft = JSON.parse(raw);
    const savedAt = Number(draft?.savedAt || 0);

    if (!draft?.designJson || !savedAt || Date.now() - savedAt > maxAgeMs) {
      window.localStorage.removeItem(LOGIN_DRAFT_KEY);
      return null;
    }
